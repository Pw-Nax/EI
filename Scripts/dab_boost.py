...
data   = np.loadtxt('electronica_industrial/boost_mod_16kw.csv', skiprows=1, delimiter=',')

# 30°
t_v   = data[:len(data)//2, 0]
vL_v  = data[:len(data)//2, 1]
v1_v  = data[:len(data)//2, 2]
v2r_v = data[:len(data)//2, 3]
iL_v  = data[:len(data)//2, 4]
vo_v  = data[:len(data)//2, 5]
io_v  = data[:len(data)//2, 6]

for i in range(0,len(t_v)):
    # t_v[i] = (t_v[i] - 9) * 1e6 # Boost
    t_v[i] = (t_v[i] - 1) * 1e6 # Buck
...
# ax3.yaxis.set_major_locator(MultipleLocator(400)) # Boost
...
# ax4.yaxis.set_major_locator(MultipleLocator(7.5)) # Boost 3kW
# ax4.yaxis.set_major_locator(MultipleLocator(10)) # Boost 4kW
ax4.yaxis.set_major_locator(MultipleLocator(63)) # Buck 3kW
# ax4.yaxis.set_major_locator(MultipleLocator(83)) # Buck 4kW
